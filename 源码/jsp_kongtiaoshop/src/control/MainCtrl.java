	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ac.equals("login")) {
			...
			if (list.size() == 1) {
				...
				gor("/kongtiaoshop/admin/index.jsp", request, response);
			} else {
			...
			if (member != null) {
				...
				if (addrlist.size() == 0) {
					...
					HashMap gmap = dao.select(
							"select * from kongtiao where id=" + gid).get(0);
		...
		if (ac.equals("updatetprice")) {
			ArrayList<HashMap> goodlist = (ArrayList<HashMap>) dao
					.select("select *,a.id as aid,b.id as bid from car a,kongtiao b where a.gid=b.id and a.mid='"
							+ member.get("id")
		...
		if (ac.equals("submitorder")) {
			...
			for (HashMap carmap : carlist) {
				HashMap goodmap = dao.select(
						"select * from kongtiao where id=" + carmap.get("gid"))
						.get(0);
		...
		if (ac.equals("ddfh")) {
			...
			ArrayList<HashMap> dddetaillist = (ArrayList<HashMap>) dao
					.select("select * from dddetail where ddno="
							+ ddmap.get("ddno"));
			boolean flag = true;// 用作订单空调库存校验结果
			for (HashMap dddetailmap : dddetaillist) {
				// 如果其中某个空调的数量大于其库存量 则置 FLASE标识
			...
			if (flag) {
				...
				request.setAttribute("info", "订单中空调库存不足，发货失败!");
				go("/admin/ddgl.jsp", request, response);
			}
		}
		...
		if (ac.equals("kongtiaoadd")) {
			try {
				...
				dao.commOper("insert into kongtiao (goodno,goodname,fid,sid,price,note,savetime,shstatus,filename,istj,tprice,delstatus,salestatus,goodpp) "
						+ "values ('"
						+ goodno
						+ "','"
						+ goodname
						...
						+ "','0','在售','" + goodpp + "') ");
				request.setAttribute("suc", "操作成功!");
				go("/admin/kongtiaogl.jsp", request, response);

			} catch (Exception e1) {
				e1.printStackTrace();
				request.setAttribute("error", "");
				request.getRequestDispatcher("/admin/kongtiaogl.jsp").forward(
						request, response);
			}

		}
		// 会员修改空调
		if (ac.equals("kongtiaoedit")) {
			String id = request.getParameter("id");
			HashMap map = dao.select("select * from kongtiao where id=" + id).get(
					0);
			try {
				String goodname = "";
				String fid = "";
				...
				dao.commOper("update kongtiao set goodname='" + goodname
						+ "',fid='" + fid + "',sid='" + sid + "',price='"
						+ price + "',note='" + note + "',filename='" + filename
						+ "',goodpp='" + goodpp + "' where id=" + id);
				request.setAttribute("suc", "操作成功!");
				go("/admin/kongtiaogl.jsp?id=" + id, request, response);
			} catch (Exception e1) {
				e1.printStackTrace();
				request.setAttribute("error", "");
				request.getRequestDispatcher("/admin/kongtiaogl.jsp").forward(
		...
		if (ac.equals("kongtiaokcadd")) {
			...
			dao.commOper("insert into kcrecord (gid,happennum,type,savetime) values "
					...
					+ savetime + "') ");
			request.setAttribute("suc", "操作成功!");
			go("/admin/kongtiaokc.jsp", request, response);
		}
		...
		if (ac.equals("goodpj")) {
			...
			String note = request.getParameter("note");
			String savetime = Info.getDateStr();
			dao.commOper("insert into kongtiaopj (goodid,memberid,note,savetime) values ('"
					+ goodid
					+ "','"
					+ memberid
					+ "','"
					...
					+ savetime + "')");
			request.setAttribute("suc", "");
			go("/kongtiaox.jsp?id=" + goodid, request, response);
		}
		...
		if (ac.equals("pj")) {
			...
			String savetime = Info.getDateStr();
			HashMap mm = dao.select("select * from kongtiao where id=" + goodid)
					.get(0);
			dao.commOper("insert into pj (goodid,kongtiaoaver,memberid,jb,msg,savetime,hf) values "
					+ "('"
					+ goodid
					+ "','"
					+ mm.get("saver")
					...
					+ savetime
					+ "','') ");
			request.setAttribute("suc", "空调评价成功!");
			go("/mydd.jsp", request, response);
		}
		// 管理员回复评价
		if (ac.equals("pjhf")) {
		...
		if (ac.equals("addfav")) {
			...
			if (member == null) {
				...
				if (cklist.size() == 0) {
					...
					request.setAttribute("suc", "此空调已收藏过!");
					go("/" + page, request, response);
				}
			}
		}
		...
		if (ac.equals("kongtiaotjset")) {
			String id = request.getParameter("id");
			String tprice = request.getParameter("tprice") == null ? ""
					: request.getParameter("tprice");
			if (!tprice.equals("")) {
				HashMap map = dao.select("select * from kongtiao where id=" + id)
						.get(0);
				if (Double.valueOf(tprice) >= Double.valueOf(map.get("price")
						.toString())) {
					request.setAttribute("info", "特价必须低于原价!");
					go("/admin/kongtiaotjset.jsp?id=" + id, request, response);
				} else {
					dao.commOper("update kongtiao set tprice='" + tprice
							+ "' where id=" + id);
					request.setAttribute("suc", "特价设置成功!");
					go("/admin/kongtiaogl.jsp", request, response);
				}
			} else {
				dao.commOper("update kongtiao set tprice='' where id=" + id);
