...
<html lang="en">
  ...
  <body class="">
    ...
    <%	 CommDAO dao = new CommDAO(); 
     String sql = "select * from kongtiao where 1=1 and delstatus='0' ";
	 String url = "/kongtiaoshop/admin/kongtiaogl.jsp?1=1";
			    ...
			    <div class="alert alert-info">
			        <button type="button" class="close" data-dismiss="alert">×</button>
			        	...
			        	<script type="text/javascript">
			        		location.href="admin/kongtiaogl.jsp";
			        	</script>
		    	</div>
			<%}%>
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="admin/kongtiaogl.jsp?f=f" class="form-inline">
							<input class="input-xlarge" placeholder="空调编号或名称..." id="key" name="key" type="text" value="<%=key %>">
						    ...
						    <button class="btn btn-primary" onclick="kongtiaoadd()" type="button"><i class="icon-plus"></i> 空调</button>
						</form>	
					</div>  	
				</div>
...
<div class="well">
    <table class="table">
      ...
      if(did!=null)
      {
   	   dao.commOper("update kongtiao set delstatus='1' where id="+did);
      }
      String tjid = request.getParameter("tjid");
      if(tjid!=null)
      {
   	   dao.commOper("update kongtiao set istj='yes' where id="+tjid);
      }
      String qxtjid = request.getParameter("qxtjid");
      if(qxtjid!=null)
      {
   	   dao.commOper("update kongtiao set istj='no' where id="+qxtjid);
      }
   ...
   ArrayList<HashMap> list=(ArrayList)bean.getCollection();
   	for(HashMap map:list){
        ...
        <tr>
          ...
          <a href="admin/kongtiaotjset.jsp?id=<%=map.get("id") %>">特价</a>
          </td>
          ...
          <td>
              <%
			    ...
			    <a href="admin/kongtiaogl.jsp?tjid=<%=map.get("id") %>" title="限时抢购"><i class="icon-thumbs-up"></i></a>
			    <%}else{ %>
			    <a href="admin/kongtiaogl.jsp?qxtjid=<%=map.get("id") %>" title="取消限时抢购"><i class="icon-thumbs-down"></i></a>
			    <%} %>
              <a href="admin/kongtiaoedit.jsp?id=<%=map.get("id") %>"><i class="icon-pencil"></i></a>
              <a href="admin/kongtiaogl.jsp?did=<%=map.get("id") %>" ><i class="icon-remove"></i></a>
          </td>
        </tr>
        <%} %>
      </tbody>
...
<div class="pagination">
    ...
    <script type="text/javascript">
    	function kongtiaoadd(){
			location.href="admin/kongtiaoadd.jsp";
        }
        $("[rel=tooltip]").tooltip();
        $(function() {
        ...
        function Change_Select(){//当第一个下拉框的选项发生改变时调用该函数
          ...
          var url = "/kongtiaoshop/kongtiaoshop?ac=searchsontype&fprotype="+ escape(fprotype);
